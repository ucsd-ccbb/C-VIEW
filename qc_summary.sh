...
python $PIPELINEDIR/samtools_depth_violinplot.py $WORKSPACE/*/*.depth.txt && mv depth_violin.pdf $WORKSPACE/qc
python $PIPELINEDIR/samtools_depth_lineplot.py $WORKSPACE/*/*.depth.txt && mv depth_lineplot.pdf $WORKSPACE/qc
python $PIPELINEDIR/samtools_depth_concat.py $WORKSPACE/*/*.depth.txt > $WORKSPACE/qc/depth.tsv
python $PIPELINEDIR/samtools_depth_low.py 266 29674 10 $WORKSPACE/*/*.depth.txt > $WORKSPACE/qc/depth_below_10.tsv
python $PIPELINEDIR/consensus_acceptance_summary.py 

# Multiqc
multiqc --config $PIPELINEDIR/covid_custom_config.yaml $WORKSPACE
