...
cat $WORKSPACE/"$SAMPLEID".trimmed.sorted.pileup.variants.tsv | cut -f11 | grep -v "^ALT_FREQ" | numlist -gte0.05 | numlist -lte0.95 | wc -l > $WORKSPACE/"$SAMPLEID".numlist.txt ) ; } > $WORKSPACE/"$SAMPLEID".log.11.diversity.log 2>&1
