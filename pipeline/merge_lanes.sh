...
if [[ ! ${#SAMPLES_WO_LANE_INFO[@]} == ${#SAMPLES_W_LANES_COMBINED[@]} ]] ; then
   echo "Error: must have same number of sample-without-lane and sample-with-lane-combined entries" >> merge.log
   exit 1
fi
...
do
  ...
  echo $SAMPLE_W_LANES_COMBINED >> merge.log
...
aws s3 cp $WORKSPACE/"$SEQ_RUN"_lane_merged_fastq/ $S3DOWNLOAD/"$SEQ_RUN"_lane_merged_fastq/ --recursive --exclude "*" --include "*.log"
