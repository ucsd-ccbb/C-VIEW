...
KEY_ORDER = [SEARCH_ID_KEY, INSPECT_SAMPLE_ID_KEY, EXCITE_BARCODE_KEY,
             SEQ_POOL_COMP_ID, PANGO_STATUS_KEY, PANGO_LINEAGE_KEY,
             UNCAPPED_READS_KEY, PCT_30_KEY, MAPPED_READS_KEY,
             SUB_MAP_PCT_ALIGN_KEY]
...
def make_bespoke_outputs(arg_list):
    ...
    full_df.to_csv(all_output_fp, index=False)

    sources = full_df[SOURCE_KEY].unique()
    special_sources = []
    special_df = None
    for curr_source in sources:
        curr_source_name = curr_source
        curr_mask = full_df[SOURCE_KEY] == curr_source
        # in pandas, can't select records on " == nan"--nans need special case
        if pd.isna(curr_source):
            curr_source_name = "no-source"
            curr_mask = full_df[SOURCE_KEY].isna()
        curr_source_df = full_df[curr_mask].copy()
